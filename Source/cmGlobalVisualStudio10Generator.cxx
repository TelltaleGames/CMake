  ...
  : cmGlobalVisualStudio8Generator(cm, name, platformName)
{
  ...
  if( strcmp(platformName.c_str(), "XBox 360") == 0 )
  {
	  this->DefaultPlatformToolset = "2010-01";
  }  
...
bool cmGlobalVisualStudio10Generator::FindVCTargetsPath(cmMakefile* mf)
{
  ...
  std::string const vcxproj = "VCTargetsPath.vcxproj";
  {
    ...
    xw.StartElement("PropertyGroup");
	 ...
	 if (this->GetPlatformName() == "NX64") {
		 xw.StartElement("NintendoSdkRoot");
		 xw.Content("$(NINTENDO_SDK_ROOT)");
		 xw.EndElement(); // NintendoSdkRoot
		 xw.StartElement("NintendoSdkSpec");
		 xw.Content("NX");
		 xw.EndElement(); // NintendoSdkSpec
	 }
    xw.EndElement(); // PropertyGroup
	if (this->GetPlatformName() == "NX64") {
		xw.StartElement("Import");
		xw.Attribute("Project",
			"$(NintendoSdkRoot)\\Build\\Vc\\NintendoSdkVcProjectSettings.props");
...
{
  if(this->GetPlatformName() == "PS3") { return cmVS10PS3FlagTable; }
  if(this->GetPlatformName() == "PSVita") { return cmVS10PSVitaFlagTable; }
  if(this->GetPlatformName() == "ORBIS") { return cmVS10PSVitaFlagTable; }
...
{
  if(this->GetPlatformName() == "PS3") { return cmVS10PS3LinkFlagTable; }  
